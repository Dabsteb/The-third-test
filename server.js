...
app.post('/api/generate', async (req, res) => {
    if (!fetch) {
        return res.status(500).json({ error: 'Сервер еще не готов, попробуйте через секунду.' });
    }
    ...
    try {
        const apiResponse = await fetch(apiUrl, {
            ...
            body: JSON.stringify({
                model: "deepseek-chat",
                messages: [
                    // Весь промпт от клиента, включающий системные инструкции, передаем в роли 'user'
                    // Это самый простой способ интеграции без изменения фронтенда.
                    // Модель достаточно умна, чтобы правильно интерпретировать инструкции в этом формате.
        ...
        // Адаптируем ответ от DeepSeek под формат, который ожидает фронтенд (как от Gemini)
